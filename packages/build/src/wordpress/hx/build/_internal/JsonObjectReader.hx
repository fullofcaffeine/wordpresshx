...
class JsonObjectReader {
	...
	public static function from(value:JsonValue, label:String, code:String):JsonObjectReader {
		return switch value {
			case ObjectValue(fields): new JsonObjectReader(fields, label);
	...
	public function exact(expected:Array<String>, code:String):Void {
		...
		sortedExpected.sort(compareText);
		if (actual.join("\n") != sortedExpected.join("\n")) {
			invalid(code, label + " fields must be exactly [" + sortedExpected.join(", ") + "]");
		}
	}
	...
	public function optionalString(name:String, code:String):Null<String> {
		return has(name) ? string(name, code) : null;
	}
	...
	public function object(name:String, code:String):JsonObjectReader {
		return JsonObjectReader.from(require(name, code), label + "." + name, code);
	}
