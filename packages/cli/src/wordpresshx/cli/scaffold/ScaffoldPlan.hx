...
class ScaffoldPlan {
	...
	public function new(mode:ScaffoldMode, projectId:String, displayName:String, packageName:String, entryPoint:String, profile:String, targetRoot:String,
			targetName:String, files:Array<ScaffoldFile>) {
		this.mode = mode;
	...
	public function json(dryRun:Bool, published:Bool):JsonValue {
		return ScaffoldJson.object([
			...
			ScaffoldJson.field("kind", ScaffoldJson.text("site")),
			...
			ScaffoldJson.field("limitations", ScaffoldJson.array([
				ScaffoldJson.text("native-target-producers-not-registered"),
				ScaffoldJson.text("public-package-installation-blocked")
			]))
		]);
	}
