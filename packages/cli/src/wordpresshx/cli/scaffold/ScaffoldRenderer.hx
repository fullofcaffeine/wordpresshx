...
class ScaffoldRenderer {
	...
	static function readme(kind:ScaffoldKind, projectId:String, profile:String):String {
		final finalParagraph = switch kind {
			...
			case Plugin:
				"The zero-argument `WordPress.plugin()` declaration derives conventional plugin metadata from the project identity. Pass an inline typed options object only for metadata or behavior you need to override; `titleFilter: Site.filterTitle` compiles that typed Haxe method into an isolated private PHP closure behind a generated native WordPress adapter. Plain `wphx dev` also derives a fresh WordPress runtime, installs and activates the generated plugin, and reloads only after complete builds; use `wphx dev --services=none` for compile/watch-only work. General hooks, lifecycle APIs, and public package installation remain dependency-gated.\n";
		};
		return "# "
