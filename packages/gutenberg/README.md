...
## Typed static edit/save and durable migrations
...
public static function edit(props:EditProps<CalloutAttributes>):BrowserNode {
  return <aside {...BlockProps.edit({className: "wphx-callout--editor"})}>
    <PlainText
      ariaLabel="Callout message"
      value={props.attributes.message}
      onChange={next ->
        EditAttributes.set(props, selected -> selected.message, next)}
    />
  </aside>;
}
...
public static function save(props:SaveProps<CalloutAttributes>):BrowserNode {
  return <aside {...BlockProps.save({className: "wphx-callout"})}>
    <p class="wphx-callout__message">{props.attributes.message}</p>
  </aside>;
}
```

Every current and deprecated attribute has an explicit default, so missing and
`null` cannot blur together. Ordered `StaticBlock.deprecated(...)` records have
their own old attribute type, exact old save function, exact old-to-current
migration, and optional typed eligibility predicate. The generated browser
registration and generated `block.json` must agree on the block name and full
attribute schema before publication.

Follow the beginner walkthrough in
[`test/static-block-fixture`](test/static-block-fixture/README.md). Its gate
proves deterministic Genes TSX, strict TypeScript, official WordPress bundles,
native Gutenberg parse/serialize/replay bytes, six compile failures, real
insert/edit/undo/redo/save/reload, public frontend markup, and an old-content
migration with no recovery prompt:

```sh
bash packages/gutenberg/scripts/test-static-block.sh
```

Use `--skip-wordpress` for the compiler and native package proof without the
containerized WordPress editor.
