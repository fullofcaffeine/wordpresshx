...
tooling_root="${2:-}"
evidence_root="${3:-}"
plugin_slug="wordpresshx-sdk061-static-block"
project_name="${WORDPRESSHX_COMPOSE_PROJECT_NAME:-wordpresshx-sdk061}"

if [[ ! -f "${plugin_root}/blocks/callout/block.json" \
  || ! -f "${plugin_root}/blocks/callout/build/editor.js" \
  || ! -f "${tooling_root}/run-static-block-playwright.mjs" \
  || ! -d "${evidence_root}" ]]; then
  echo "usage: $0 <generated-plugin-root> <tooling-root> <evidence-root>" >&2
  exit 2
fi
...
for command_name in docker python3; do
  if ! command -v "${command_name}" >/dev/null 2>&1; then
    echo "SDK-061 real static block lane requires ${command_name}" >&2
    exit 1
  fi
done
...
"${compose[@]}" cp \
  "${package_root}/test/static-block-runtime/wordpresshx-sdk061-static-block.php" \
  "${wordpress_service}:/var/www/html/wp-content/plugins/${plugin_slug}/${plugin_slug}.php" >&2
"${compose[@]}" cp \
  "${package_root}/test/static-block-runtime/setup.php" \
  "${wordpress_service}:/opt/wordpresshx/setup-static-block.php" >&2
setup_json="$("${compose[@]}" exec --no-TTY "${wordpress_service}" php /opt/wordpresshx/setup-static-block.php "${plugin_slug}")"
IFS='|' read -r current_post_id legacy_post_id < <(
  python3 - "${setup_json}" <<'PY'
import json
import sys

payload = json.loads(sys.argv[1])
if payload.get("check") != "wordpresshx-sdk061-static-block-setup-v1":
...
if payload.get("pluginActive") is not True or payload.get("wordpressVersion") != "7.0":
    raise SystemExit(f"unexpected static block runtime setup: {payload!r}")
print(f"{payload['currentPostId']}|{payload['legacyPostId']}")
PY
)
...
docker run --rm --network "${network_name}" --ipc=host \
  ...
  node run-static-block-playwright.mjs \
    http://wordpress-mysql "${current_post_id}" "${legacy_post_id}" \
    /evidence/static-block-editor.png /evidence/wordpress-static-block.json

test -s "${evidence_root}/static-block-editor.png"
test -s "${evidence_root}/wordpress-static-block.json"
cleanup
trap - EXIT
echo "SDK-061 real WordPress static block lane passed"
