#!/usr/bin/env python3
"""Emit the native WordPress plugin described by the SDK-064 asset plan."""
...
STORE_KEY = re.compile(r"[a-z][a-z0-9-]+(?:/[a-z][a-z0-9-]+)+\Z")
...
def validate(plan: dict[str, object]) -> None:
    exact_keys(
        ...
        "data-store asset plan",
    )
    assert plan["schemaVersion"] == 1
    assert plan["profileId"] == "wp70-release"
    assert plan["dataCatalogRevision"] == "wp70-release/data-store-v1"
    plugin = plan["plugin"]
    editor = plan["editor"]
    ...
    for value in (plugin, editor, store, source, script, translations):
    ...
    exact_keys(
        store,
        {"key", "actionTypeCount", "stateFields", "nativeApis"},
        "data store",
    )
    exact_keys(
        source,
    ...
    assert STORE_KEY.fullmatch(store["key"])
    assert store["actionTypeCount"] == 6
    assert store["stateFields"] == sorted(set(store["stateFields"]))
    assert store["nativeApis"] == sorted(set(store["nativeApis"]))
    ...
    assert source["haxeEntry"] == (
        "test/data-store-fixture/src/sdk064/fixture/Main.hx"
    )
...
// Generated from the WordPressHx SDK-064 data-store plan. Do not edit.
...
\t\t\tthrow new UnexpectedValueException( 'Invalid generated WordPressHx data-store asset metadata.' );
\t\t}}
\t\twp_register_script(
\t\t\t'{script['handle']}',
...
\t\t) ) {{
\t\t\tthrow new RuntimeException( 'Unable to attach generated WordPressHx data-store translations.' );
\t\t}}
...
def translation_document(plan: dict[str, object]) -> dict[str, object]:
    ...
    return {
        ...
        "generator": "WordPressHx SDK-064 deterministic data-store emitter",
...
def emit(plan_path: Path, bundle_root: Path, output_root: Path) -> None:
    ...
    manifest = {
        "schemaVersion": 1,
        "generator": "wordpresshx-sdk064-data-store-emitter-v1",
