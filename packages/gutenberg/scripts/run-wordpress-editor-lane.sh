...
tooling_root="${3:-}"
evidence_root="${4:-}"
project_name="${WORDPRESSHX_COMPOSE_PROJECT_NAME:-wordpresshx-sdk063}"
...
for command_name in docker python3; do
  if ! command -v "${command_name}" >/dev/null 2>&1; then
    echo "SDK-063 real editor lane requires ${command_name}" >&2
    exit 1
  fi
done
...
IFS='|' read -r plugin_slug plugin_name sidebar_name supported_post_type < <(
...
for identity in "${plugin_slug}" "${plugin_name}" "${sidebar_name}"; do
  if [[ ! "${identity}" =~ ^[a-z0-9]+(-[a-z0-9]+)*$ ]]; then
    echo "unsafe generated editor identity: ${identity}" >&2
    exit 2
  fi
done
if [[ "${supported_post_type}" != "post" || ! -f "${plugin_root}/${plugin_slug}.php" ]]; then
  echo "generated editor plugin shape is invalid" >&2
  exit 2
fi
...
"${compose[@]}" cp "${package_root}/test/editor-plugin-runtime/setup.php" "${wordpress_service}:/opt/wordpresshx/setup-editor.php" >&2
setup_json="$("${compose[@]}" exec --no-TTY "${wordpress_service}" php /opt/wordpresshx/setup-editor.php "${plugin_slug}")"
...
if payload.get("check") != "wordpresshx-sdk063-editor-setup-v1":
...
if payload.get("pluginActive") is not True or payload.get("wordpressVersion") != "7.0":
    raise SystemExit(f"unexpected editor runtime setup: {payload!r}")
...
docker run --rm --network "${network_name}" --ipc=host \
  ...
  node run-editor-playwright.mjs \
    http://wordpress-mysql "${post_id}" "${page_id}" "${plugin_name}" \
    /evidence/editor-sidebar.png

test -s "${evidence_root}/editor-sidebar.png"
test -s "${evidence_root}/editor-sidebar.png.json"
cleanup
trap - EXIT
echo "SDK-063 real WordPress editor lane passed"
