...
class PhpTypedAstLowerer {
		...
		return switch (TypeTools.toString(functionData.ret)) {
			...
			case "Bool":
				...
				final parameters = if (hasRequiredNullableStringParameter(functionData)) {
					...
					lowerRequiredParameters(functionData, "String", PhpStringType);
				} else {
	...
	function lowerBoolValue(expression:TypedExpr):PhpExpr {
		return switch (expression.expr) {
			...
			case TBinop(OpLte, left, right) if (isExactStringValue(left) && isExactStringValue(right)):
				PhpSemanticCapabilities.requireAdmitted(StringLessOrEqual);
				PhpBinop("<=", PhpFunctionCall("\\strcmp", [lowerStringValue(left), lowerStringValue(right)]), PhpInt(0));
			case TBinop(OpGt, left, right) if (isExactStringValue(left) && isExactStringValue(right)):
				PhpSemanticCapabilities.requireAdmitted(StringGreaterThan);
				PhpBinop(">", PhpFunctionCall("\\strcmp", [lowerStringValue(left), lowerStringValue(right)]), PhpInt(0));
			case TBinop(OpGte, left, right) if (isExactStringValue(left) && isExactStringValue(right)):
				PhpSemanticCapabilities.requireAdmitted(StringGreaterOrEqual);
				PhpBinop(">=", PhpFunctionCall("\\strcmp", [lowerStringValue(left), lowerStringValue(right)]), PhpInt(0));
		...
		return switch (target.expr) {
			case TField(_, FStatic(classRef, fieldRef)) if (sources.owns(classRef.get().pos) && TypeTools.toString(call.t) == "Bool"):
				if (isRequiredNullableStringBoolField(classRef.get(), fieldRef.get())) {
					...
					PhpSemanticCapabilities.requireAdmitted(StaticApplicationStringPredicateCall);
					PhpStaticCall(className(classRef.get()), fieldRef.get().name, arguments.map(lowerStringValue));
				} else {
	...
	function lowerCondition(expression:TypedExpr, intArrayLengths:Map<Int, Int>):{expression:PhpExpr, mappingKind:String} {
		return switch (expression.expr) {
			case TBinop(OpEq | OpNotEq, left, right) if (isExactStringValue(left) && isExactStringValue(right)):
				final equality = switch (expression.expr) {
					case TBinop(OpEq, _, _): true;
					case _: false;
				};
				PhpSemanticCapabilities.requireAdmitted(equality ? StringEquality : StringInequality);
				{
					expression: PhpBinop(equality ? "===" : "!==", lowerStringValue(left), lowerStringValue(right)),
					mappingKind: equality ? "if-string-equality" : "if-string-inequality"
				};
