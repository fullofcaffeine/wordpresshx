	...
	public static function records():Array<PhpSemanticCapabilityRecord> {
		...
		return [
			...
			record(ArrayCollection, ValuesCollections, UnsupportedOwned,
				"general Array runtime behavior beyond fixed Int literals, exact length, proven reads/writes, and direct result-discarded push is rejected",
				owner),
			...
			record(HaxeStdlib, RuntimeStdlib, UnsupportedOwned,
				"only exact Int-array length/read/write, direct result-discarded Int-array push, and the compiler-owned Unicode-scalar String length helper exist",
				owner),
