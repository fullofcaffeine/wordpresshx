...
- `unsafe-boundary-policy.json` is ADR-019's proposed closed waiver,
  ...
  unwaivable. Its bounded validator and canonical synthetic waiver pass locally
  and in the focused hosted workflow; independent Oracle review remains
  pending.
...
- `dev-loop-implementation.json` records SDK-044's real compile/watch core:
  ...
  records the implemented POSIX process-group boundary, its SDK-owned stable
  group-leader sentinel, and exact-Node local child/grandchild plus rapid
  payload-exit regressions separately from the pending hosted Linux rerun and
  unimplemented Windows Job Object adapter. Windows external services fail
...
Receipt `SDK-051-PLUGIN-LIFECYCLE` records typed standard-plugin and must-use
