...
All notable project changes will be recorded here once versioning policy is accepted. The format is provisional while the SDK remains pre-release.
...
- Exact non-null Haxe String `<`, `<=`, `>`, and `>=` ordering in the generic PHP compiler.
  ...
  ordering for numeric-looking and multi-byte UTF-8 values. Null operands,
  coercion, locale rules, and normalization remain outside this
  claim.

### Release status

No artifact has been released and no compatibility claim has been made.
