...
assert adr009_receipt["status"] in {
    "implemented-hosted-pending",
    "verified",
...
assert output_context_architecture["claims"]["architectureDecision"] == (
    output_context_architecture["status"]
)
...
assert adr019_verification["waiverDigest"] == (
    "1bdb9dd7a04ef266e02ce860a6a30bac34fd85ce04ae3e5199a40947bde5bf5a"
)
assert adr019_verification["categoryCount"] == 9
assert adr019_verification["scenarioCount"] == 14
assert adr019_verification["failClosedMutationCount"] == 60
assert adr019_verification["independentSchemaCount"] == 4
assert adr019_verification["independentSchemaInstanceCount"] == 8
assert adr019_verification["independentSchemaWeakeningProbeCount"] == 8
...
adr019_correction_review = adr019_receipt["review"][
    "firstCorrectionOracleRereview"
]
assert adr019_correction_review["decision"] == "changes-required"
assert adr019_correction_review["reviewedCommit"] == (
    "05421557763d61b3bd87996714508227da619db2"
)
assert adr019_correction_review["packetSha256"] == (
    "2cedd63d13f5c22699cae4adae98ca884fcb2fba9b307e6028b399581dcd6a49"
)
assert adr019_correction_review["resolvedFindings"] == [
    "ADR019-F002",
    "ADR019-F004",
]
assert adr019_correction_review["openFindings"] == [
    "ADR019-F001",
    "ADR019-F003",
    "ADR019-RF001",
    "ADR019-RF002",
...
for adr019_review_path, adr019_review_digest in (
    (
        adr019_correction_review["reportPath"],
        adr019_correction_review["reportSha256"],
    ),
    (
        adr019_correction_review["decisionPath"],
        adr019_correction_review["decisionSha256"],
    ),
    (
        adr019_correction_review["reviewedInputsPath"],
        adr019_correction_review["reviewedInputsSha256"],
    ),
):
    assert hashlib.sha256(Path(adr019_review_path).read_bytes()).hexdigest() == (
        adr019_review_digest
...
if adr019_hosted["status"] in {
    ...
    "pending-second-oracle-correction-hosted-run",
}:
    ...
    if adr019_hosted["status"] == "pending-second-oracle-correction-hosted-run":
        assert isinstance(adr019_hosted["lastPassedRunId"], int)
        assert isinstance(adr019_hosted["lastPassedJobId"], int)
        assert sha1.fullmatch(adr019_hosted["lastPassedCommit"])
...
assert sdk026_policy_sha256 == (
    "81721f1b8518b05e15f10175c4002c31fb3a8e1b3c1591a2335d213704656141"
)
...
assert php_quality_toolchain["security"] == {
    "remediations": [
        ...
        },
        {
            ...
            "resolution": "squizlabs/php_codesniffer@3.13.6",
        },
    ],
}
...
assert sdk026_receipt["toolchain"]["phpCodeSnifferSecurityRemediation"] == {
    "advisory": "GHSA-hmqg-cxww-wqhq",
    "cve": "CVE-2026-67434",
    "affectedVersions": "<3.13.6|>=4.0.0,<4.0.2",
    "resolvedVersion": "3.13.6",
}
...
assert reflaxe_php_semantic_receipt["localEvidence"][
    ...
    "current-string-greater-or-equal-subject-passed-run-32653961861; "
    "predecessor-string-greater-than-subject-passed-run-32599678054"
)
...
assert reflaxe_php_semantic_receipt["hostedVerification"] == {
    "relation": "current-string-greater-or-equal-subject",
    "admittedCapabilityCount": 86,
    "workflow": "Repository bootstrap",
    "runId": 32653961861,
    "url": "https://github.com/fullofcaffeine/wordpresshx/actions/runs/32653961861",
    "commit": "071a11946d9b547aa8fc0f46eb1f7ac7e4d62c89",
    "status": "passed",
    "job": {
        "name": "haxe",
        "jobId": 97229738906,
        "url": (
            ...
            "32653961861/job/97229738906"
        ),
        "status": "passed",
        "completedAt": "2026-08-23T17:26:31Z",
    },
    ...
    "scope": (
        "the current 86 exact admitted semantic records with lexical String greater-or-equal "
        "for exact non-null String operands, a typed source-owned String predicate call, deterministic "
        "generic compiler output, a clean offline package consumer, and native PHP "
        "7.4.33/8.4.7 parser and runtime behavior only"
    ),
...
assert reflaxe_php_semantic_receipt["previousHostedVerification"] == {
    "relation": "predecessor-string-greater-than-subject",
    "admittedCapabilityCount": 85,
    "workflow": "Repository bootstrap",
    "runId": 32599678054,
    "url": "https://github.com/fullofcaffeine/wordpresshx/actions/runs/32599678054",
    "commit": "6b5c788a9a0a7171dfdd8ecb32215209c2696025",
    "status": "passed",
    "job": {
        "name": "haxe",
        "jobId": 97095975557,
        "url": (
            "https://github.com/fullofcaffeine/wordpresshx/actions/runs/"
            "32599678054/job/97095975557"
        ),
        "status": "passed",
        "completedAt": "2026-08-22T21:44:50Z",
    },
    "exactPhpVersions": ["7.4.33", "8.4.7"],
    "requiredSteps": [
        "Install Haxe 4.3.7",
    ...
    "scope": (
        "the predecessor 85 exact admitted semantic records with lexical String greater-than "
        "for exact non-null String operands, a typed source-owned String predicate call, deterministic generic compiler output, "
        "a clean offline package consumer, and native PHP 7.4.33/8.4.7 parser and "
...
assert len(reflaxe_php_semantic_input_records) == 144
...
assert reflaxe_php_module_output_receipt["hostedVerification"] == {
    "relation": "current-string-greater-or-equal-module-graph-subject",
    "workflow": "Repository bootstrap",
    "runId": 32653961861,
    "url": "https://github.com/fullofcaffeine/wordpresshx/actions/runs/32653961861",
    "commit": "071a11946d9b547aa8fc0f46eb1f7ac7e4d62c89",
    ...
    "scope": (
        "the current compiler package with lexical String greater-or-equal for exact non-null String operands "
        "and a typed source-owned String predicate call, a clean package consumer, the exact "
        "PHP runtime matrix, the deterministic WordPress package adapter, and the public "
        "PHP profile only"
    ),
    "wordpressRuntimeCompatibilityAdvanced": False,
    "officialHaxeQualificationAdvanced": False,
...
assert reflaxe_php_module_output_receipt["previousHostedVerification"] == {
    "relation": "predecessor-string-greater-than-module-graph-subject",
    "workflow": "Repository bootstrap",
    "runId": 32599678054,
    "url": "https://github.com/fullofcaffeine/wordpresshx/actions/runs/32599678054",
    "commit": "6b5c788a9a0a7171dfdd8ecb32215209c2696025",
    ...
    "scope": (
        "the predecessor compiler package with lexical String greater-than for exact non-null String operands "
        "and a typed source-owned String predicate call, a clean package consumer, the exact PHP runtime matrix, "
        "the deterministic WordPress package adapter, and the public PHP profile only"
    ),
    "wordpressRuntimeCompatibilityAdvanced": False,
    "officialHaxeQualificationAdvanced": False,
    "publicationAuthorized": False,
...
assert len(reflaxe_php_module_output_input_records) == 27
reflaxe_php_module_subject_commit = (
    reflaxe_php_module_output_receipt["hostedVerification"].get("commit")
    or reflaxe_php_module_output_receipt["previousHostedVerification"]["commit"]
)
