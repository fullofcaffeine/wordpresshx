...
test_root="$(mktemp -d "${temporary_parent}/wordpresshx-sdk043-gate.XXXXXX")"
cleanup() {
  case "${test_root}" in
    "${temporary_parent}"/wordpresshx-sdk043-gate.*) rm -rf -- "${test_root}" ;;
    *) echo "refusing to remove unexpected SDK-043 test path" >&2 ;;
  esac
}
trap cleanup EXIT
...
for command_name in docker haxe haxelib lix node python3 realpath; do
  if ! command -v "${command_name}" >/dev/null 2>&1; then
    echo "SDK-043 production CLI gate requires ${command_name}" >&2
    exit 1
  fi
done
if [[ "$(haxe --version)" != "4.3.7" ]]; then
  echo "SDK-043 production CLI gate requires host Haxe 4.3.7" >&2
  exit 1
fi
...
if [[ ! -x "${lix_haxe}" ]] || [[ "$(cd "${package_root}" && "${lix_haxe}" --version)" != "4.3.7" ]]; then
  echo "SDK-043 production CLI gate requires the exact Lix-scoped Haxe shim" >&2
  exit 1
fi
(
  cd "${package_root}"
...
(
  cd "${repository_root}/fixtures/project-cli/project"
...
python3 "${repository_root}/scripts/project-cli/test-production.py" "${test_root}/runtime-a"
