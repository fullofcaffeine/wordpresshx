...
Haxe surface, generated PHP and JavaScript facades, and exact provider artifact.
The content bundle excludes the final ownership manifest. The existing ADR-007
owner validates that bundle from staged bytes, then owns the bundle and every
member in its final manifest. This direction avoids a digest cycle and leaves
one publisher for the complete adoption.
...
the complete staged content. A staged ownership manifest with internally
