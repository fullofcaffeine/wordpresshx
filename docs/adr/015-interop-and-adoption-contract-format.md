...
the complete staged content, including both runtime trust anchors. A staged ownership manifest with internally
consistent hashes does not compensate for stale or incomplete bundle
...
Eight negative fixtures reject observation, scope, token, friend-path, and
authority-owner forgery, plus wrong capabilities, cross-target use, and omitted
members.
...
It rejects 84 layer-isolated document mutations. Each non-stale mutation gets
a fresh self-digest, so a digest mismatch cannot hide a missing semantic check.
