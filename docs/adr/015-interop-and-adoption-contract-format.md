...
One content bundle binds the contract, capability set, review report, generated
Haxe surface, and exact provider artifact. Generated PHP and JavaScript facades
are trust anchors outside that bundle. Each embeds the bundle's final digest
and verifies every member byte before it observes provider code. The bundle
excludes the facades and final ownership manifest to avoid a digest cycle. The
existing ADR-007 owner validates the complete stage and owns the bundle, every
member, and both anchors in its final manifest.
...
exact provider ID, version, target executable-closure identity, required native symbols, and
...
The PHP closure is the exact plugin source file. The browser closure is the
canonical identity of the module and package-metadata files that the adapter
captures and imports. The distribution ZIP remains provider provenance, but a
target token does not claim that its runtime observed every ZIP member.

The target adapter owns provider verification, native observation, lifecycle
scope, and token minting. It captures verified provider bytes or an equivalent
immutable handle before execution. Callers cannot replace its declared
capability requirements or supply observation facts, and a downstream class at
a former test-friend path receives no privileged access.
...
Before publication, the ADR-007 owner runs the adoption-bundle validator over
the complete staged content, including both runtime trust anchors. The
validator closure captures a non-candidate expected-stage plan from the
trusted generator invocation. This plan binds every staged path, hash, size,
closure, binding, symbol, and the final ownership manifest. An internally
consistent candidate manifest does not compensate for stale or forged staged
semantics; validation fails before the ownership transaction starts.
...
One bundle digest binds the contract, capability set, review report, generated
Haxe surface, and provider artifact. The PHP and JavaScript anchors embed and
verify that digest from outside the bundle. The separate ADR-007 manifest owns
the bundle, its members, and both anchors without joining the digest cycle.
The provider artifact is a deterministic ZIP of the runtime fixture files.

An interpreter-only test module shadows PHP-request, PHP-process, and
browser-module scope behavior without importing product authority.
...
The Python determinism and self-consistency validator checks every input,
schema, record, bundle member, and architecture rule. It rejects 84
layer-isolated document mutations. Each non-stale mutation gets a fresh
self-digest, so a digest mismatch cannot hide a missing semantic check. A
separate TypeScript-AST observer does not import the Python ABI model or
generator and checks JavaScript module syntax, exports, and formals against
the contract, capability set, and generated Haxe.
